package code.android.bill.purchases.base.repository
...
import code.android.bill.purchases.base.repository.BillingRepository.RetryPolicies.resetConnectionRetryPolicyCounter
import code.android.bill.purchases.base.repository.BillingRepository.RetryPolicies.taskExecutionRetryPolicy
import code.android.bill.purchases.base.repository.local.LocalBillingDb
...
class BillingRepository private constructor(
    ...
    companion object {
        private val TAG = "conHeoRepository"
        ...
        billingResult?.let { result ->
            when (result.responseCode) {
                BillingClient.BillingResponseCode.OK -> {
                    ...
                    Log.d(
                        TAG,
                        "onSkuDetailsResponse(): responseCode: ${billingResult.responseCode} --- message:${result.debugMessage}"
                    )
                }
            }
        }
        ...
        billingResult?.let {
            when (it.responseCode) {
                BillingClient.BillingResponseCode.OK -> {
                    Log.d(
                        TAG,
                        "onPurchaseHistoryResponse(): OK - result:$purchaseHistoryRecordList"
                    )
            ...
            mBillingClient.acknowledgePurchase(params) { billingResult ->
                when (billingResult.responseCode) {
                    BillingClient.BillingResponseCode.OK -> {
                        ...
                        Log.d(
                            TAG,
                            "onAcknowledgePurchaseResponse() response is : ${billingResult.debugMessage}"
                        )
                    }
                }
            }
